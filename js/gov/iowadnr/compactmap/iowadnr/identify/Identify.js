...
(function() {
  var css = [
    ...
    dojo.moduleUrl("dojox", "grid/resources/nihiloGrid.css"), // tundra theme for grids
...
// https://dillerdesign.com/experiment/DD_roundies/ 
if (dojo.isIE) {
  (function() {
    var js = [
...
dojo.declare("dijits.identify.Identify", [dijit._Widget, dijit._Templated], {
  ...
  // are called, see:
  // https://docs.dojocampus.org/dijit/_Widget#lifecycle
  ...
  constructor: function(params, srcNodeRef) {
    ...
    this.label = params.label || "Identify"; //Link this to header to identify active tool.
  ...
  startup: function() {
    ...
    if (dojo.isIE) {
      DD_roundies.addRule('.genid-rounded-corner', '5px');
    }
  ...
  _mapClickHandler: function(evt) {
    ...
    dojo.byId(this.id + "-count").innerHTML = "(x)";
  ...
  _gridUpdated: function() {
    ...
    // count again.
    // dojo.byId(this.id + "-count").innerHTML = "(" + this._featureGrid.rowCount + ")";
    if (this._featureGrid.rowCount === 0){
...
dojo.mixin(dijits.identify.Identify, {
  ...
  // https://docs.dojocampus.org/dojox/grid#important-information-about-formatting-and-security
